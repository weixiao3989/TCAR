//
//  Show_Passenger_EvaluationVC.swift
//  TCAR
//
...
class Show_Passenger_EvaluationVC: UIViewController, UITableViewDataSource, UITableViewDelegate {
    ...
    override func viewDidLoad() {
        ...
        // Get Passenger Past Rate.
        getPassengerPastRate()
    }
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if rateArray.count == 1 {
            ...
            cell.textLabel?.text = NSLocalizedString("SPEV_Score", comment: "") + rateArray[indexPath.row] + "   " + NSLocalizedString("SPEV_Delivery_Date", comment: "") + rateArray[indexPath.row + 1]
        } else if rateArray.count == 6 {
            cell.textLabel?.text = NSLocalizedString("SPEV_Score", comment: "") + rateArray[indexPath.row] + "   " + NSLocalizedString("SPEV_Delivery_Date", comment: "") + rateArray[indexPath.row + 2]
        } else {
            cell.textLabel?.text = NSLocalizedString("SPEV_Score", comment: "") + rateArray[indexPath.row] + "   " + NSLocalizedString("SPEV_Delivery_Date", comment: "") + rateArray[indexPath.row + 3]
        }
    ...
    private func setupNavigationBarItems() {
        navigationItem.title = NSLocalizedString("SPEV_Navigation_Title", comment: "")
    }
    
    // Get Passenger Rate.
    func getPassengerPastRate() {
        ...
        AccessAPIs.sendRequest_noParameters(url: TCAR_API.getUserInfo_URL(_id: user_id), method: .get, headers: headers) { response, error in
            ...
            DispatchQueue.main.async {
                ...
                if let ratelist = json["user"]["rate"]["detail"].array {
                ...
                switch json["user"]["rate"]["count"].intValue {
                case 0:
                    self.rateArray.append(NSLocalizedString("SPEV_AlertView_NoRate_Title", comment: ""))
                    break
                case 1:
                ...
                let average_Score = String(format: "%.1f", json["user"]["rate"]["score"].doubleValue)
    ...
    func showRateDetail(index: Int) {
        if rateArray.count == 1 {
            JSSAlertView().show(
                ...
                text: NSLocalizedString("SPEV_AlertView_NoRate_Content", comment: ""),
                buttonText: NSLocalizedString("OK", comment: ""))
        } else if rateArray.count == 3{
            JSSAlertView().show(
                self,
                title: NSLocalizedString("SPEV_GetScore", comment: "") + self.rateArray[index],
                text: NSLocalizedString("SPEV_Delivery_Date", comment: "")
                    + self.rateArray[index + 1]
                    + "\n" + NSLocalizedString("SPEV_Rate", comment: "")
                    + "\n"
