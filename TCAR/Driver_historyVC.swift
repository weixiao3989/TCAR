//
//  Driver_historyVC.swift
//  TCAR
//
//  Created by WeiXiao on 2018/1/26.
...
class Driver_historyVC: UIViewController, UITableViewDelegate, UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        let cell = tableView.dequeueReusableCell(withIdentifier: "driver_HistoryCell", for: indexPath)
        
        if self.table_Count == 0 {
            cell.textLabel?.text = NSLocalizedString("DAMV_NoHistory_Title", comment: "")
        } else {
            cell.textLabel?.text = NSLocalizedString("DAMV_History_Cell_Title", comment: "") + self.orderInfo_Array[(indexPath.row * 6) + 1]
        }
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        if self.table_Count == 0 {
            UIAlertController.OB_showNormalAlert(
                title: NSLocalizedString("DAMV_NoHistory_DialogTxt_Title", comment: ""),
                message: NSLocalizedString("DAMV_NoHistory_DialogTxt_Message", comment: ""),
                alertTitle: NSLocalizedString("Cancel", comment: ""),
                in: self)
        } else {
            let type = NSLocalizedString("DAMV_History_DialogTxt_Title", comment: "") + self.orderInfo_Array[indexPath.row * 6] + "\n"
            let startTime = NSLocalizedString("DAMV_History_DialogTxt_PickupTime", comment: "")
                + self.orderInfo_Array[indexPath.row * 6 + 1] + "\n"
            let endTime = NSLocalizedString("DAMV_History_DialogTxt_UpdatedTime", comment: "")
                + self.orderInfo_Array[indexPath.row * 6 + 2] + "\n"
            let start = NSLocalizedString("DAMV_History_DialogTxt_StartLocation", comment: "")
                + self.orderInfo_Array[indexPath.row * 6 + 3] + "\n"
            let dest = NSLocalizedString("DAMV_History_DialogTxt_Destination", comment: "")
                + self.orderInfo_Array[indexPath.row * 6 + 4] + "\n"
            let person = NSLocalizedString("DAMV_History_DialogTxt_Person", comment: "")
                + self.orderInfo_Array[indexPath.row * 6 + 5] + "\n"
            let score = NSLocalizedString("DAMV_History_DialogTxt_Score", comment: "")
                + self.rateInfo_Array[indexPath.row * 2] + "\n"
            let eva = NSLocalizedString("DAMV_History_DialogTxt_Evaluation", comment: "")
        ...
        AccessAPIs.sendRequest_noParameters(url: TCAR_API.getRTCC_status_FINISH_URL(), method: .get, headers: self.headers) { response, error in
            ...
            for tasks in task_list! {
                ...
                if tasks["type"].stringValue.contains("NOW") {
                    self.orderInfo_Array.append(NSLocalizedString("DAMV_History_DialogTxt_RTCC", comment: ""))
                } else if tasks["type"].stringValue.contains("Resevation") {
                    self.orderID_Array.append(NSLocalizedString("DAMV_History_DialogTxt_RSCC", comment: ""))
                } else {
                    self.orderID_Array.append(NSLocalizedString("DAMV_History_DialogTxt_TCCC", comment: ""))
                }
            ...
            for i in self.orderID_Array {
                AccessAPIs.sendRequest_noParameters(url: TCAR_API.getListInfo_URL(_id: i), method: .get, headers: self.headers) { rate_response, error in
                    ...
                    if let rate_list = rate["task"]["rate"].array {
                        ...
                        self.rateInfo_Array.append(NSLocalizedString("DAMV_History_DialogTxt_NoScore", comment: ""))
                        self.rateInfo_Array.append(NSLocalizedString("DAMV_History_DialogTxt_NoNote", comment: ""))
                    }
                }
            }
